...
\begin{frame}{Motivación}
  \begin{columns}[t]
    ...
    \textbf{Ajuste discreto: curva de una bomba centrífuga}
    ...
    El ensayo entrega $m$ pares $(Q_i, H_i)$ \alert{con dispersión
      experimental}: no tiene sentido pasar una curva por todos los puntos.
    Se busca
    \[ H(Q) \approx a_0 + a_1 Q + a_2 Q^2 \]
    y con ella el \textbf{punto de funcionamiento}, intersección con la curva
    resistente de la instalación $H_s = H_e + K Q^2$.

    \column{0.48\textwidth}
    \textbf{Ajuste continuo: mecanismo biela-manivela}
    ...
    \begin{center}
      \begin{tikzpicture}[
        ...
        \draw[|<->|] (2.2129,-0.62) -- (2.60,-0.62);
        \node[below] at (2.41,-0.60) {\scriptsize $x$};
      \end{tikzpicture}
    \end{center}

    El desplazamiento del pistón es una función \alert{conocida y exacta}:
    \[ x(\theta) = r(1 - \cos\theta) + l \left( 1 - \sqrt{1 - \lambda^2 \sen^2\theta} \right), \; \lambda = \frac{r}{l} \]
    La raíz complica derivar, integrar y hacer análisis armónico. Se busca la
    mejor aproximación por funciones simples en $[0, \pi]$.

  \end{columns}
  \vspace{0.4em}
  \raggedleft {\small $\mapsto$ Ver ejercicios 5 y 12 de la práctica 9.}
\end{frame}
